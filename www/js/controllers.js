  ...
  $scope.show_loading = function() {
    ...
    $http.post(link, {role_id : $scope.userInfo.role ,userid: $scope.userInfo.userid,is_completed : $scope.selection.togglecomplete,starttime:start_date,endtime:end_date}).then(function (res){
		...
		for(var i =0; i<res.data.length;i++){
			...
			list.preview_image = res.data[i].preview_image;
...
.controller('controller_course_trainer', function($sce,$rootScope, $scope, $ionicLoading,$http, $state, $timeout, $ionicPopup, $stateParams,webservice,$cordovaGeolocation) {
...
.controller('controller_course_employee', function($sce,$rootScope, $scope, $ionicLoading,$http, $state, $timeout, $ionicPopup, $stateParams,webservice,$cordovaGeolocation) {
    var courseid = $stateParams.programId;
    var link = webservice.principal + webservice.urls.programByID;
	...
	$scope.refreshdatafeed = function(){
		$scope.show_loading();
		$http.post(link_getUnitsByCourse, {userid: $scope.userInfo.userid,courseid: courseid}).then(function (res){
			$scope.hide_loading();
			$scope.units = res.data;
			for(var i = 0;i< res.data.length;i++){
				var b = i;
				...
				//NO OLVIDAR CAMBIAR
				var d = new Date;
				if($scope.units[i].saved == 0 && $scope.units[i].date == d.toISOString().slice(0,10)){
		...
		var posOptions = {timeout: 10000, enableHighAccuracy: false};
		 $cordovaGeolocation
			...
			.then(function (position) {
				...
				$http.post(link_saveattendance, {userid: $scope.userInfo.userid, courseid: courseid, action: 'confirm',location: lat+','+long, unitid:unitid,inarea:inarea}).then(function (res){
					alert("Confirmed");
					$scope.refreshdatafeed();
				});
			}, function(err) {
